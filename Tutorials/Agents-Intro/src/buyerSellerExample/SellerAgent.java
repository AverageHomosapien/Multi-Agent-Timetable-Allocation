...
public class SellerAgent extends Agent{
	...
	// Synchronising daily behaviours
	public class TickerWaiter extends CyclicBehaviour {
		...
		// Generates the daily stock available
		public class BookGenerator extends OneShotBehaviour {
			
			@Override
			public void action() {
				booksForSale.clear();
				int rand = (int)Math.round((1 + 2 * Math.random())); // Select 1 book per sale per day
				int price = (int)Math.round((1 + 49 * Math.random())); // Price between 1 and 50 GBP
			...
			public void action() {
				...
				try {
					...
					for (int i = 0; i < agentsType1.length; i++) {
						buyers.add(agentsType1[i].getName()); // Got buyers AIDs
					}
				}
	...
	// Serves an offer for the requested book - listens for calls for proposals from buyers
	public class OffersServer extends CyclicBehaviour {
		...
		public void action() {
			...
			if (msg != null) {
				...
				if (booksForSale.containsKey(book)) { // we can send an offer if we have the book
				...
				myAgent.send(reply); // Refuse or propose
			}
			else {
				block();
			}
	...
	// Waits for each buyer to be done
	// Behaviours are removed after receives done from all buyers
